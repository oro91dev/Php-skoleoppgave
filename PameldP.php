...
include 'Ovelse_publikumklasse.php';
?>
<form  method="Post">
    <table>
<?Php
...
?>
   <form method="POST">
   <td>Finn Billettype: 
   <select size="1" name="Billettype">

 <?Php
 while($row=mysql_fetch_assoc($resultat)){
 ?>
   <option value="<?php echo $row['Billettype']?>">
       <?Php echo $row['Billettype'];?>
   </option>
<?Php
 }
?>
...
function valider_ovelse($ovelse)
{
    ...
    if(!$ovelse->getBillettype())
    {
        $ovelse->setBillettype("");
...
if(isset($_REQUEST["Reg"]))
{
    ...
    $ovelse = new Ovelse_publikumklasse($_REQUEST['Fornavn'],$_REQUEST['Etternavn'],$_REQUEST['Dato'],$_REQUEST['Tid'],$_REQUEST['Type'],$_REQUEST['Billettype']);
    $feilstring = valider_ovelse($ovelse);
    
    if($feilstring==null)
    {
         
        $feilstring .=$ovelse->lagre_Ovelse_publikumklasse_db();
    }
    else 
     {
